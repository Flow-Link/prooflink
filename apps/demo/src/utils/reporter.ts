...
export class DemoReporter {
  ...
  saveHtmlReport(outputPath?: string): string {
    ...
    const filePath = outputPath ?? "/tmp/flowlink-demo-report.html";
...
<head>
  ...
  <title>FlowLink Demo Report — ${escapeHtml(report.scenarioName)}</title>
  <style>
...
<body>
  <div class="container">
    ...
    <div class="footer">
      Generated by FlowLink Demo Runner &mdash; ${new Date().toISOString().split("T")[0]}<br>
      <a href="https://flowlink.finance" style="color:#06b6d4;text-decoration:none;">flowlink.finance</a>
    </div>
  </div>
</body>
</html>`;
